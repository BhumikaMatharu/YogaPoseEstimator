...
def main(args):
    ...
    dict = {'bridge': 1, 'childs': 2, 'downwarddog': 3, 'mountain': 4, 'plank': 5, 'seatedforwardbend': 6, 'tree': 7, 'trianglepose': 8, 'warrior1': 9, 'warrior2': 10}
            ...
            if not file.startswith('NO_'):
                ...
                for person_id in tqdm(list(tracking_results.keys())):
                    ...
                    for i in range(len(output_dict['joints3d'])):
                        ...
                        len_N = len(flat_arr)
                        np.savetxt(joints3D_csv, [np.append(flat_arr, [video_label])], delimiter=',', fmt=' '.join(['%f']*len_N + ['%i']))
                        
                    for i in range(len(output_dict['pose'])):
                        pose_arr = output_dict['pose'][i].flatten()
                        len_M = len(pose_arr)
                        np.savetxt(pose_csv, [np.append(pose_arr, [video_label])], delimiter=',', fmt=' '.join(['%f']*len_M + ['%i']))
